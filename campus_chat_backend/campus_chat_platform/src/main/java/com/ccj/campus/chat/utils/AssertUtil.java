...
import com.ccj.campus.chat.exception.ErrorEnum;
...
public class AssertUtil {
    ...
    public static <T> void fastFailValidate(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            throwException(CommonErrorEnum.INVALID_PARAM, constraintViolations.iterator().next().getMessage());
        }
    }
    ...
    public static <T> void allCheckValidateThrow(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            ...
            //去掉最后一个逗号
            throwException(CommonErrorEnum.INVALID_PARAM, errorMsg.toString().substring(0, errorMsg.length() - 1));
        }
    }
    ...
    //如果不是非空对象，则抛异常
    public static void isNotEmpty(Object obj, String errMsg) {
        if (isEmpty(obj)) {
    ...
    //如果不是非空对象，则抛异常
    public static void isEmpty(Object obj, String errMsg) {
        if (!isEmpty(obj)) {
            throwException(errMsg);
        }
    }
