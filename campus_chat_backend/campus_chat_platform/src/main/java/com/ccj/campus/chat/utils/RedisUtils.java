...
//                    "end ";
    ...
    public static <T> T executeScript(String scriptPath, Class<T> resClass, String key, Object... args) {
        DefaultRedisScript<T> redisScript = new DefaultRedisScript<>();
        redisScript.setResultType(resClass);
        redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource(scriptPath)));
        return stringRedisTemplate.execute(redisScript, Collections.singletonList(key), args);
    }

    public static <T> T executeScript(String scriptPath, Class<T> resClass, List<String> keys, Object... args) {
        DefaultRedisScript<T> redisScript = new DefaultRedisScript<>();
        redisScript.setResultType(resClass);
        redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource(scriptPath)));
    ...
    public static String executeScript(String scriptPath, List<String> keys, Object... args) {
    ...
    public static void executePipeline(SessionCallback<Object> sessionCallback) {
    ...
    public static Boolean setIfAbsent(String key, Object value, long timeSeconds) {
        try {
            if (timeSeconds > 0) {
                return stringRedisTemplate.opsForValue().setIfAbsent(key, objToStr(value), timeSeconds, TimeUnit.SECONDS);
            }
            return stringRedisTemplate.opsForValue().setIfAbsent(key, objToStr(value));
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return false;
        }
    ...
    static <T> T toBeanOrNull(String json, Class<T> tClass) {
        ...
        return JsonUtils.toObj(json, tClass);
    }
     ...
     * @return true成功 false失败
     */
    public static Boolean hmset(String key, Map<String, ?> map, long time, TimeUnit timeUnit) {
        try {
            stringRedisTemplate.opsForHash().putAll(key, map);
            if (time > 0) {
    ...
    public static Long sSetOne(String key, String value) {
        try {
    ...
    public static <IN> Long sSetAndParser(String key, List<IN> values, Function<IN, String> parser) {
        try {
            String[] data = values.stream().map(parser).toArray(String[]::new);
            return stringRedisTemplate.opsForSet().add(key, data);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            return 0L;
        }
    ...
    public static <IN> Long sSetAndParser(String key, Set<IN> values, Function<IN, String> parser) {
        try {
            String[] data = values.stream().map(parser).toArray(String[]::new);
            return stringRedisTemplate.opsForSet().add(key, data);
        } catch (Exception e) {
     ...
     * @return
     */
    public static void zAdd(String key, Set<TypedTuple<String>> values) {
